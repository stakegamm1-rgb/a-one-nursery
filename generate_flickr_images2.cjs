...
function createProduct(name, category) {
  ...
  // Real photos from Flickr based on tags, locked to the ID so they don't change on refresh
  const image = `https://loremflickr.com/800/800/${encodeURIComponent(name.split(' ')[0])},plant/all?lock=${idCounter}`;
...
console.log("Successfully generated 50 plants with real, stable Flickr images.");
