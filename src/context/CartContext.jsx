...
export const CartProvider = ({ children }) => {
  const [cart, setCart] = useState(() => {
    ...
    if (saved) {
      ...
      return parsedCart.map(cartItem => {
        const freshPlant = plants.find(p => p.id === cartItem.id);
        return freshPlant ? { ...freshPlant, qty: cartItem.qty } : cartItem;
      });
    }
    return [];
  });
  ...
  const [wishlist, setWishlist] = useState(() => {
    ...
    if (saved) {
      ...
      return parsedWishlist.map(wItem => {
        const freshPlant = plants.find(p => p.id === wItem.id);
        return freshPlant ? freshPlant : wItem;
      });
    }
    return [];
  });
